module kana_aggregator::aggregatorP2 {
  ...
  fun check_and_deposit_opt<X>(sender: &signer, coin_opt: Option<coin::Coin<X>>) {
    ...
    fun check_and_deposit<X>(sender: &signer, coin: coin::Coin<X>) acquires Platform {
        let sender_addr = signer::address_of(sender);
 ...
 public entry fun direct_route<X, Y, E>(
    ...
    public entry fun update_platform_fee(account: &signer, rec:address, fee:u64) acquires Platform{
         assert!(fee <= MAX_FEE, E_MORETHAN_MAX_FEE);
        let admin_addr = signer::address_of(account);
        assert!(admin_addr == @kana_aggregator, E_NOT_ADMIN);

        if(!exists<FeeProfile>(admin_addr)){
